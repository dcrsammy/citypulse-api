...
router.post('/social/:friend_id/send', auth, async (req, res) => {
  try {
    ...
    const message = {
      ...
      venue_category: req.body.venue_category || null,
      venue_neighbourhood: req.body.venue_neighbourhood || null,
      venue_rating: req.body.venue_rating || null,
...
router.post('/system/:chat_id/send', auth, async (req, res) => {
  try {
    ...
    const msgRef = await firebase.ref(`chats/${req.params.chat_id}/messages`).push(message);
    await firebase.ref(`chats/${req.params.chat_id}/metadata`).update({
      last_message: text,
      last_sender: req.user.id,
      updated_at: Date.now(),
    });
